...
import { auth, db } from '../firebase'; // Asegúrate de que esta ruta apunte a tu firebase.ts

export const PortalAsesores = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. El Guardia de Seguridad (Validar credenciales)
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      const user = userCredential.user;

      // 2. El Archivero (Buscar el perfil y el rol)
      ...
      if (userDoc.exists()) {
        ...
        // 3. Abrir la puerta hacia Agendar Visita
        navigate('/agendar-visita'); // Ajusta a la ruta que tengas en tu App.tsx
      } else {
