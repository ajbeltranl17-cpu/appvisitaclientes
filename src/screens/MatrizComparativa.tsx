  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto py-6 space-y-6">
        ...
        {/* CONTENEDOR DE TABLA CON SCROLL HORIZONTAL (Solución Móvil) */}
        <div className="w-full overflow-x-auto pb-8 snap-x scroll-smooth">
          <div className="min-w-max px-6">
            <table className="w-full border-separate" style={{ borderSpacing: '0 0' }}>
              <thead>
                <tr>
                  {/* Columna Izquierda Fija: Se redujo a 90px en celular */}
                  <th className="sticky left-0 z-20 bg-gray-50 w-[90px] md:w-40 border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)]"></th>
                  
                  {/* Tarjetas de Propiedades */}
                  {propiedadesAComparar.map(prop => (
                    <th key={prop.id} className="min-w-[260px] md:min-w-[320px] px-2 align-bottom snap-center">
              ...
              <tbody>
                {/* FILA: PRECIO */}
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 py-6 px-2 text-[10px] md:text-xs font-black text-[#00213b] uppercase tracking-widest border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] align-middle">Precio</td>
                  {propiedadesAComparar.map(prop => (
                ...
                {/* FILA: PRECIO / M2 */}
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 py-6 px-2 text-[10px] md:text-xs font-black text-[#00213b] uppercase tracking-widest border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] align-middle">Precio / M²</td>
                  {propiedadesAComparar.map(prop => (
                ...
                {/* FILA: PLUSVALÍA */}
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 py-6 px-2 text-[10px] md:text-xs font-black text-[#00213b] uppercase tracking-widest border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] align-middle">Plusvalía Esp.</td>
                  {propiedadesAComparar.map(prop => (
                ...
                {/* FILA: COMPATIBILIDAD */}
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 py-6 px-2 text-[10px] md:text-xs font-black text-[#00213b] uppercase tracking-widest border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] align-middle">Compatibilidad</td>
                  {propiedadesAComparar.map(prop => (
                ...
                {/* FILA: AMENIDADES */}
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 py-6 px-2 text-[10px] md:text-xs font-black text-[#00213b] uppercase tracking-widest border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] align-middle">Amenidades</td>
                  {propiedadesAComparar.map(prop => (
                    <td key={`amenidades-${prop.id}`} className="bg-white border-b border-gray-100 text-center py-6 px-4">
                      <div className="flex justify-center gap-3">
                         {prop.amenidades.map((icon, i) => (
                           <div key={i} className="w-10 h-10 rounded-xl bg-gray-50 text-[#00213b] flex items-center justify-center border border-gray-100">
                ...
                <tr>
                  <td className="sticky left-0 z-10 bg-gray-50 w-[90px] md:w-40 p-2 border-r border-transparent shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)]"></td>
                  {propiedadesAComparar.map(prop => (
                    <td key={`btn-${prop.id}`} className="bg-white p-5 rounded-b-3xl border border-t-0 border-gray-100 shadow-sm align-top">
                       <button 
